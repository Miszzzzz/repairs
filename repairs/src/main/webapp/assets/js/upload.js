...
$(function(){
    ...
    $('#btn-submit-upload').on('click',function(){
        ...
        const ordinaryData = $("#myForm").serializeArray();
        console.log("==========");
        console.log(ordinaryData);
        $.each(ordinaryData,function (i,field) {
            formFile.append(field.name,field.value);
        })
        $.each(imgFile, function(i, file){
            formFile.append('myFile-'+i, file);
        });
        console.log(imgFile);
        console.log(formFile.get("appointmentDate"));
        console.log(formFile.get("myFile-0"));
        $.ajax({
            url:"saveRepairs",
            processData:false,
            contentType:false,
